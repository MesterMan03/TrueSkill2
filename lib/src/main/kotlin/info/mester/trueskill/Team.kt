...
data class Team(
     ...
     */
    fun withPlayers(newPlayers: List<Player>): Team = copy(players = newPlayers)

    /**
     * Creates a copy of this team with an updated rank.
     */
    fun withRank(newRank: Int): Team = copy(rank = newRank)

    override fun toString(): String = "Team(rank=$rank, players=[${players.joinToString(", ") { it.id }}])"
}
