...
data class Rating(
     ...
     */
    fun applyDynamics(timeDelta: Double, dynamicsFactor: Double): Rating {
        val newVariance = standardDeviation * standardDeviation + 
                         timeDelta * dynamicsFactor * dynamicsFactor
        return Rating(mean, sqrt(newVariance))
    }
    
    override fun toString(): String {
        return "Rating(μ=${"%.3f".format(mean)}, σ=${"%.3f".format(standardDeviation)})"
    }
    ...
    companion object {
         ...
         */
        fun defaultRating(initialMean: Double = 25.0, initialStdDev: Double = 25.0 / 3.0): Rating {
