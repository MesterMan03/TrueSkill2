...
class TrueSkillThroughTime(
    ...
    fun updateRatings(
        ...
        val teamsWithDynamics = match.teams.map { team ->
    ...
    fun processMatchHistory(matches: List<Match>): List<Match> {
        ...
        for (match in matches) {
            ...
            val teamsWithCurrentRatings = match.teams.map { team ->
                val playersWithRatings = team.players.map { player ->
     ...
     */
    fun getPlayerHistory(playerId: String, processedMatches: List<Match>): List<TimestampedRating> {
     ...
     */
    fun predictFutureRating(player: Player, currentTime: Long, futureTime: Long): Rating {
     ...
     */
    private fun applyDynamicsToTeam(team: Team, lastTime: Long, currentTime: Long): Team {
     ...
     */
    private fun calculateTimeDelta(fromTime: Long, toTime: Long): Double {
    ...
    companion object {
         ...
         */
        fun withTimeUnit(timeUnit: TimeUnit, config: TrueSkillConfig = TrueSkillConfig.default()): TrueSkillThroughTime {
            return TrueSkillThroughTime(config, timeUnit)
        }
    }
}
