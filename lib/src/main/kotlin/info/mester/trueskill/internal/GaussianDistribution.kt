...
internal object GaussianDistribution {
    ...
    fun standardNormalInverseCdf(p: Double): Double {
        ...
        if (abs(q) <= 0.425) {
            ...
            return q * (((((((2.5090809287301226727e3 * r + 3.3430575583588128105e4) * r
                    + 4.3162944234493772205e4) * r + 5.7107332567861765700e4) * r
                    + 6.7265770927008700853e4) * r + 4.5921953931549871457e4) * r
                    + 1.3731693765509461125e4) * r + 1.9715909503065514427e3) /
                    (((((((1.0 * r + 7.7425252513113380888e3) * r
                            + 2.0529762245432132916e4) * r + 3.9307895800092710610e4) * r
                            + 5.2872526033990542176e4) * r + 4.8612393403931612035e4) * r
                            + 2.9246445603710159190e4) * r + 6.7183008615061117220e3)
        }
        ...
        val result = if (s <= 5.0) {
            ...
            ((((((7.74545014278341407640e-4 * t + 2.27238449892691845833e-2) * t
                    + 2.41780725177450611770e-1) * t + 1.27045825245236838258e0) * t
                    + 3.64784832476320460504e0) * t + 5.76949722146069140550e0) * t
                    + 4.63033784615654529590e0) /
                    (((((1.0 * t + 7.71154519324537209953e-2) * t
                            + 7.04916420258654092892e-1) * t + 3.32624458947632290653e0) * t
                            + 8.08924570494082762506e0) * t + 8.45791721868324965501e0)
        } else {
            val t = s - 5.0
            ((((((2.01033439929228813265e-7 * t + 2.71155556874348757815e-5) * t
                    + 1.24266094738807843860e-3) * t + 2.65321895265761230930e-2) * t
                    + 2.96560571828504891230e-1) * t + 1.78482653991729133580e0) * t
                    + 5.46378491116411436990e0) /
                    (((((1.0 * t + 2.04426310338993978564e-15) * t
                            + 1.42151175831644588870e-7) * t + 1.84631831751005468180e-5) * t
                            + 7.86869131145613259100e-4) * t + 1.48753612908506148525e-2)
        }
     ...
     * V(t, ε) = φ(t-ε) / Φ(t-ε)
     */
    fun vFunction(t: Double, epsilon: Double): Double {
        val x = t - epsilon
     ...
     */
    fun wFunction(t: Double, epsilon: Double): Double {
        val x = t - epsilon
     ...
     */
    fun multiply(mean1: Double, stdDev1: Double, mean2: Double, stdDev2: Double): Pair<Double, Double> {
     ...
     */
    fun divide(mean1: Double, stdDev1: Double, mean2: Double, stdDev2: Double): Pair<Double, Double> {
        val precision1 = 1.0 / (stdDev1 * stdDev1)
        val precision2 = 1.0 / (stdDev2 * stdDev2)
